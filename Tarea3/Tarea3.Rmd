...
```{r}
#Calculamos el valor máximo que puede tener gamma para comparar 
gamma_max = 1/min(c(d_out, d_in))

#El score del nodo i es la suma de la fila ith.
Kscore1 = rowSums(K_hat1) 
Tscore1 = rowSums(T_hat1) 

#Comparación algoritmo HITS (Mirar correlaciones entre scores, si los rankings son 
#iguales y si ambas puntuaciones convergen con gammas elevada)
...
Podemos observar que con una $\gamma$ aleatoria, en este ejemplo `r grado`, la correlación entre la $K_{\gamma}$ y el Authorities de HITS es elevada (`r cor`).
...
En este caso, observamos que la correlación es mínima (`r cor2`). Teóricamente, para valores más altos de $\gamma$, NK tendría que converger a HITS, pero en este caso, el resultado no es el esperado, puesto que la correlación indica que los valores de NK y Auth se alejan más entre ellos.


**(b) How does the decay factor $\gamma$ affect the output of the Neumann Kernel algorithm?**

El factor de difusión ($\gamma$) da menos importancion o puntuación cuanto más lejos está la conexión entre dos nodos. Es decir, tiene mayor peso la conexión entre dos vértices directamente conectados que los que tienen *n* aristas entre ellos. La teoria dice que las matrices de KN converge si $\gamma \geq \frac{1}{min(\Delta^-,\Delta^+)}$, donde $\Delta^-$ es el máximo grado entrada y $\Delta^+$ es el máximo grado de salida grafo. Cuando se elige un valor elevado de $\gamma$ la matriz $K_{\gamma}$ equivale al authorities de HITS.
