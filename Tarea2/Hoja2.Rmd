...
compute_graph_random = function(g, n){
  for(i in 1:n+1){
    sample = sample(V(g), perc)
    g = delete_vertices(g, sample)
    avrg_connected = c(avrg_connected, count_components(g))
    avrg_largest_component = c(avrg_largest_component, max(components(g)$csize))
  }

  sprintf("Media componentes conectos %s", mean(avrg_connected))
  sprintf("Media componente más largo conecto %s", mean(avrg_largest_component))
}


compute_graph_random(g, n)
...
compute_graph_degree = function(g, n){
  for(i in 1:n+1){
    
    v_center = max(centr_degree(g)$res)
    print("Pos nodo más degree")
    print(v_center)
    print("Nombre nodo más degree")
    print(V(g)[1045])
    g = delete_vertices(g, v_center)
    avrg_connected = c(avrg_connected, count_components(g))
    avrg_largest_component = c(avrg_largest_component, max(components(g)$csize))
  }
